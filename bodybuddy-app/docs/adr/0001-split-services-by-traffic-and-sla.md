...
| `analysis-worker` | Worker | SQS 소비, mock OCR, 점수 계산 | 지연 허용, retry 가능 |
...
- Helm chart, 배포, 로그, 메트릭 대상이 네 개로 늘어난다.
- 로컬 실행과 환경변수 관리가 단일 서버보다 복잡하다.

## Validation

- EKS에서 네 서비스 모두 `1/1 Running`으로 배포됐다.
- `user-service -> SQS -> analysis-worker -> score-service` 흐름으로 score가 반영됐다.
