...
이 문서는 BodyBuddy dev 환경에서 수행한 부하 테스트 결과와 튜닝 과정을 기록한다.

이 부하 테스트의 목표는 단순히 "버틴다"를 보여주는 것이 아니라, 아래를 숫자로 남기는 것이다.
...
- `evidence/08-load-test/01-score-service-hpa-scale-out.png`
- `evidence/08-load-test/02-ranking-read-second-run-result.png`
...
![Score service HPA scale-out](./evidence/08-load-test/01-score-service-hpa-scale-out.png)

2차 ranking-read 결과:

![Ranking read second run result](./evidence/08-load-test/02-ranking-read-second-run-result.png)
...
- 즉 이번 측정의 첫 번째 개선 스토리는 "캐시 로직 수정"보다 **CPU 기반 scale-out 활성화만으로 tail latency를 크게 줄였다**는 것이다.
